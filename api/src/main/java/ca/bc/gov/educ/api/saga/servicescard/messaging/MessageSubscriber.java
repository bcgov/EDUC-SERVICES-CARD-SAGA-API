...
import ca.bc.gov.educ.api.saga.servicescard.exception.ServicesCardSagaRuntimeException;
import ca.bc.gov.educ.api.saga.servicescard.orchestrator.SagaEventHandler;
import ca.bc.gov.educ.api.saga.servicescard.props.ApplicationProperties;
...
public class MessageSubscriber {
  ...
  public MessageSubscriber(final ApplicationProperties applicationProperties) throws IOException, InterruptedException {
    Options options = new Options.Builder()
            ...
            .clientId("pen-request-saga-subscriber" + UUID.randomUUID().toString())
      ...
      connection.subscribe(topic, queue, (Message message) -> {
        if (message != null) {
          ...
          try {
            ...
            log.error("Exception during subscription of event.", e);
          }
        }
      }, options);
    } catch (IOException | InterruptedException | TimeoutException e) {
      throw new ServicesCardSagaRuntimeException(e.getMessage());
    }
  }
    ...
    if (e != null) {
      ...
      while (true) {
        try {
          ...
          break;
        } catch (IOException | InterruptedException | ServicesCardSagaRuntimeException ex) {
